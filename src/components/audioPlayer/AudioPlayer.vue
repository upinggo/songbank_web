<template>
  <div
    class="player"
    :class="{
      'player-loading': isPlaying && isLoading
    }"
  >
    <div class="player-body">
      <div class="player-info">
        ...
        <div class="player-music">
          <div class="player-title">{{ currentMusic.title || 'Untitled' }}</div>
          <div class="player-author">{{ currentMusic.artist || 'Unknown' }}</div>
        </div>
      </div>
    </div>
...
<script type="text/babel">
  import Vue from 'vue'
  import Controls from './PlayerController.vue'
  import { deprecatedProp } from '../../utils.js'
  ...
  const AudioPlayer = {
    ...
    props: {
      ...
      /**
       * @since 1.4.0
       * whether to show native audio controls below Vue-player
    ...
    data () {
      return {
        internalMusic: this.list[this.music],
        isPlaying: false,
        isSeeking: false,
    ...
    computed: {
      ...
      playIndex: {
        get () {
          let idx = this.shuffledList.indexOf(this.currentMusic)
          return idx === -1 ? 0 : idx
        },
        set (val) {
          const idx = val % this.shuffledList.length
          this.currentMusic = this.shuffledList[idx]
          console.log(val + ':' + this.shuffledList.length + ':' + idx)
          this.$emit('indexChange', idx)
        },
      },
      shouldRepeat () {
    ...
    methods: {
      ...
      nextSong () {
          this.playIndex++
          if(this.playIndex >= this.shuffledList.length)
            this.playIndex = 0
          this.thenPlay()
      },
      preSong () {
      ...
      onAudioTimeUpdate () {
        ...
        this.$emit('progressChange',  this.playProgress)
      },
      onAudioSeeking () {
        this.playStat.playedTime = this.audio.currentTime
      },
    ...
    watch: {
      music (music) {
        this.internalMusic = this.list[music]
        if(this.isPlaying) this.thenPlay()
      },
      list (list) {
        if(list && list.length )
        {
          let idx = list[this.internalMusic]
          if(idx === -1) this.internalMusic = list[0]
        }
        this.shuffledList = this.getShuffledList()
      },
      playing (val) {
        if(val !== this.isPlaying) this.toggle();
      },
      progress (val) {
        if(val !== this.playProgress) {
          this.onProgressDragging(val)
          this.onProgressDragEnd(val)
        }
      },
      // list (val) {
      //   this.shuffledList = [...val]
      // },
      currentMusic: {
        handler (music) {
          const src = music.src
    ...
    beforeCreate () {
      if (!AudioPlayer.disableVersionBadge && !versionBadgePrinted) {
